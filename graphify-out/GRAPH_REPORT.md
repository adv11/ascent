...
## Corpus Check
- 337 files · ~293,311 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1427 nodes · 3122 edges · 124 communities (83 shown, 41 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 44 edges (avg confidence: 0.7)
- Token cost: 516,056 input · 0 output
...
5. `navigate()` - 25 edges
6. `FirebaseAdapter` - 22 edges
7. `attachFocusTrap()` - 22 edges
8. `test` - 21 edges
9. `withTimeout()` - 20 edges
10. `createBrandMark()` - 20 edges
...
Cohesion: 0.06
Nodes (46): RFC-4180, RFC-5545, buildEvent(), buildTodosIcs(), escapeIcsText(), foldLine(), formatIcsDate(), pad() (+38 more)
...
Cohesion: 0.06
Nodes (43): ADR-008: Backup export/import schema versioning strategy, Exact-match-or-reject schema versioning (EXPORT_SCHEMA_VERSION), activityLog day-count map vs item.completedAt distinction, ADR-009: Progress analytics data model (completedAt vs activityLog), buildEffectiveActivityLog() backfill for pre-existing history, onCompletionToggle(delta) dependency-injected hook, CORRUPTION_MARKERS, extractItemTitleText() (+35 more)

### Community 5 - "Feedback Metadata & Rate Limiting"
Cohesion: 0.07
Nodes (33): collectCurrentMetadata(), collectMetadata(), parseBrowser(), parseOs(), canSubmit(), getSubmitLog(), msUntilNextSubmit(), recordSubmit() (+25 more)

### Community 6 - "E2E Test Suite"
Cohesion: 0.06
Nodes (8): CONTRAST_FALSE_POSITIVE_SELECTORS, runAxe(), seriousOrCritical(), createCustomRoadmapViaImport(), minimalImportJson(), createCustomRoadmapViaImport(), minimalImportJson(), test

### Community 7 - "Review Scheduling & Celebration State"
Cohesion: 0.08
Nodes (34): getReviewDueItems(), groupReviewDueItemsByTag(), isReviewDue(), hasShownPhaseCelebration(), hasShownRoadmapCelebration(), markPhaseCelebrationShown(), markRoadmapCelebrationShown(), read() (+26 more)

### Community 8 - "Package Dependencies"
Cohesion: 0.04
Nodes (44): @axe-core/playwright, eslint, @eslint/js, firebase-tools, globals, jsdom, description, devDependencies (+36 more)

### Community 9 - "Brand & Auth Marketing Panel"
Cohesion: 0.09
Nodes (34): ICONS, iconSvg(), VALUE_PROPS, brandGlyph(), createBrandIcon(), createBrandMark(), createBrandWordmark(), createProgressRing() (+26 more)

### Community 10 - "Auth & Account Guards"
Cohesion: 0.11
Nodes (24): RFC-5322, assertAccountDeletable(), assertHasPasswordCredential(), signOutWithCleanup(), app, auth, authApi, authErrorMessage() (+16 more)
...
Cohesion: 0.19
Nodes (21): createAvatar(), initialsFor(), openDeleteAccountModal(), createEmptyState(), createIcon(), openMyReports(), createSidebar(), createSkeletonCard() (+13 more)

### Community 16 - "Time Tracking & Daily Todo Panel"
Cohesion: 0.16
Nodes (12): accumulateElapsed(), computeElapsedSeconds(), formatTimeSpent(), createDailyTodoPanel(), ICON_SHAPES, VALID_SIZES, openItemPanel(), createSelect() (+4 more)
...
Cohesion: 0.13
Nodes (9): bindCommandPaletteShortcut(), fuzzyMatch(), openCommandPalette(), openModal(), navigationItems(), navigate(), routes, setup() (+1 more)

### Community 19 - "Daily Todo Limits & Activity Log Store"
Cohesion: 0.16
Nodes (9): clampDurationMs(), DURATION_PRESETS, createActivityLogStore(), DEFAULT_STREAK_FREEZES, ADR-0009, pruneOldEntries(), createDailyTodoStore(), getStorageAdapter() (+1 more)
...
Cohesion: 0.21
Nodes (11): activeRoadmapTitle(), copyToClipboard(), openShareRoadmapModal(), renderShareRow(), shareUrl(), ensureRoot(), showToast(), buildPickingOverlay() (+3 more)

### Community 24 - "PWA Offline Cache Strategy"
Cohesion: 0.21
Nodes (11): Cache-first strategy for static assets, Network-first with stale-cache fallback for Firebase requests, ADR-011: PWA offline caching strategy, cacheFirst(), FIREBASE_API_HOSTS, isFirebaseApiRequest(), networkFirst(), findClientToFocus() (+3 more)
...
Cohesion: 0.21
Nodes (9): openAddToDailyTodoModal(), openBuildYourOwnGuide(), openChangelogDrawer(), renderEntryItem(), renderVersionGroup(), TYPE_LABELS, openDailyTodoGuide(), attachFocusTrap() (+1 more)
...
Cohesion: 0.15
Nodes (15): Brand rules — createBrandMark()/createBrandWordmark()/createBrandIcon(), apis.google.com / frame-src allowlist entry (issue #168, gapi cross-tab auth iframe), Content Security Policy (CSP), ADR-002: CSP + SRI security hardening, Firebase SDK upgrade process (sync import URL + SRI hash x3), Subresource Integrity (SRI), themeBootstrap.js external-script extraction rationale (no-inline-script CSP), Background sync rejected (Firebase SDK bypasses service worker fetch) (+7 more)
...
Cohesion: 0.28
Nodes (13): dismissInstallPrompt(), isInstallable(), listeners, onInstallabilityChange(), promptInstall(), getTheme(), onThemeChange(), toggleTheme() (+5 more)
...
Cohesion: 0.16
Nodes (12): activityLogStore, app, dailyTodoStore, feedbackWidget, guardApp(), lazyGuard(), store, registerServiceWorker() (+4 more)

### Community 33 - "Confirm Dialog & Sign-out Utils"
Cohesion: 0.25
Nodes (5): confirmDialog(), setButtonLoading(), confirmAndSignOut(), confirmSignOutDespiteFailedFlush(), flushDirtyStores()
...
Cohesion: 0.25
Nodes (9): apply(), hasExplicitPreference(), initTheme(), media, setTheme(), subscribers, systemTheme(), freshTheme() (+1 more)
...
Cohesion: 0.36
Nodes (6): Brand component consolidation (createBrandMark/createBrandIcon/createBrandWordmark), localStorage key migration (switchprep-* to ascent-*), ADR-004: Product rename to Ascent, migrateLocalStorageKeys(), RENAMES, PAIRS
...
## Knowledge Gaps
- **237 isolated node(s):** `name`, `version`, `private`, `license`, `type` (+232 more)
...
- **Why does `el()` connect `Component Library (avatar/empty state/skeleton/sidebar)` to `Analytics Engine`, `Backup/Export Schema + ICS Export`, `Confirm Dialog & Sign-out Utils`, `Feedback Report Schema + Share Schema`, `Import Validation & Corruption Detection`, `Feedback Metadata & Rate Limiting`, `Review Scheduling & Celebration State`, `Brand & Auth Marketing Panel`, `Auth & Account Guards`, `Changelog & Feature Badges`, `Share Card Canvas Rendering`, `Time Tracking & Daily Todo Panel`, `Command Palette & Router`, `Tabs Component`, `Share/Delete Account Modals`, `Guide & Changelog Drawer Components`, `Chart Wrapper Component`, `PWA Install & Theme Service`?**
  _High betweenness centrality (0.121) - this node is a cross-community bridge._
- **Why does `CLAUDE.md (root agent instructions)` connect `Root Docs (CLAUDE.md/AGENTS.md/ADR-001/ADR-007)` to `Issue Tracker Sync Workflow`, `Issue Templates & Docs Index`, `Responsive Breakpoint ADR-006`, `CI Workflow Jobs`, `Public API Docs`, `CSP/SRI Security ADRs`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `StorageAdapter` connect `LocalStorage Adapter` to `Feedback Report Schema + Share Schema`?**
