...
| `completeTour` | `async () => void` | Issue #17 — sets `tourDone = true`, persists to `users/{uid}/meta/tourDone` (plus a local fallback). Called by `featureTour.js`'s `onEnd` callback whether the tour was skipped, closed via Escape, or completed by reaching "Done" on the last step — all three count as "the tour is over." |
| `resetTour` | `() => void` | Issue #17 — sets `tourDone = false` **in memory only**, for a manual "Take a tour" replay (`sidebar.js`). Does not persist until the replay itself is skipped/completed again (`completeTour()` re-fires at that point). |
...
| `createCustomRoadmap` | `async ({ title, description?, phases?, items? }) => Promise<string>` | Issue #4 (extended for AI-import). Generates a `croadmap-...` id, appends `{ id, title, description, createdAt }` to `customRoadmaps`, then activates it via `switchRoadmap(id, { customRoadmaps })` — `customRoadmaps` is persisted as part of that call's single meta write, not a separate `saveMeta` round trip first (issue #121 item 6). `phases`/`items` are populated from `adaptImportToRoadmap()`'s output on every call site since issue #100 retired the manual "start truly blank" flow that used to omit them — the roadmap seeds empty only if a future caller ever omits them again (stashed in an internal one-shot `pendingCustomSeeds` map consumed by `fetchTemplateData`; that branch is otherwise unreachable dead code today). Throws if `title` is empty/whitespace-only. **Issue #324**: also throws a tagged `Error` (`error.code === 'capped'`) once `customRoadmaps.length` reaches `MAX_CUSTOM_ROADMAPS` (25, `core/roadmap/limits.js`) — checked against `customRoadmaps.length` only, never `startedTemplateIds.length`, so the 7 built-in templates never count toward it. Callers should check `error.code` to show a dedicated "you've hit the limit" message rather than a generic failure. Returns the new id. |
| `getAllRoadmapsForSearch` | `async () => { id: string, title: string, items: Record<string, Item>, phases: TemplatePhase[] }[]` | Issue #283 — read-only cross-roadmap query for the command palette's global topic search. Resolves every roadmap in `startedTemplateIds` (built-in + custom) via the same cache → Firebase → local blob → seed resolution `resolveRoadmapItems()`/`fetchTemplateData()` already use, run concurrently (`Promise.all`). **Never writes to `roadmapCache` or any other store state** — unlike every other `resolveRoadmapItems()` caller, this only reads, so opening the command palette can't accidentally cache a roadmap the user hasn't switched to. Feeds `src/core/roadmap/globalTopicSearch.js`'s `searchTopicsAcrossRoadmaps(roadmaps, query)`, a pure function (no DOM/store access) matching on title/phase/section/notes/resource label+url, ranked title-first, capped at 20 results by default. See `.claude/rules/roadmap-store.md`. |
| `getRoadmapSnapshotForComparison` | `async (templateId: string) => { items: Record<string, Item>, phases: TemplatePhase[] }` | Issue #285 — read-only snapshot of *any* started roadmap (not necessarily the active one), for `roadmapComparisonModal.js`'s "compare against another roadmap I've started" mode. Thin wrapper around `resolveRoadmapItems(templateId, fetchTemplateData(templateId))` — same cache-first resolution order as every other roadmap load, but never assigns into `activeTemplateId`/`items`/`roadmapCache`/`dirty` and never attaches a listener. Returns `{ items: {}, phases: [] }` if `templateId` is falsy. The actual diff is computed by `src/core/roadmap/roadmapComparison.js`'s `compareRoadmapTopics(itemsA, itemsB)` (pure, matches topics by a normalized `(phase, title)` key via `comparisonKey()`, not by item id) and `groupComparisonByPhase(comparison)`. The "compare against my own starter template" mode needs no store call at all — it diffs `store.getSnapshot().items` directly against `buildSeedItems(templateId)` from the template registry. See `.claude/rules/roadmap-store.md`. |
...
```ts
{
  ...
  tourDone: boolean | null, // issue #17 — null only before the first setUser() resolves;
                            // gates the first-time feature tour's auto-start
                            // (dashboard.js starts it iff onboardingDone === true &&
                            // tourDone === false)
}
```
...
| `markStarted` | `(id: string) => boolean` | Issue #555. First-start-only — sets `startedAt` to `Date.now()` once; returns `false` (no-op) for a missing id or one that's already started. Called from `dailyTodoPanel.js`'s timer-start handler, not on every (re)start. |
...
## `src/core/analytics/progressDigest.js` — weekly progress digest (issue #284)

Pure — no DOM/store access. Kept separate from `computeAnalytics()` above since the
dashboard's digest banner only needs two numbers and is evaluated on every mount, not
worth paying for heatmap/projection/priority-breakdown work just to decide whether to
show a one-line banner. `activityLog` should be the effective (backfilled) log, same as
every other analytics consumer — see `buildEffectiveActivityLog()` above.

| Export | Signature | Notes |
|---|---|---|
| `computeWeeklyCompletedCount` | `(activityLog, now?) => number` | Sum of `activityLog` counts over the trailing 7 calendar days (today inclusive) — same trailing window `computeVelocity()` uses, summed instead of averaged. |
| `computeProgressDigest` | `(activityLog, now?, frozenDates?) => { completedCount, streakDays }` | `streakDays` is `computeStreaks()`'s `current` — can never disagree with the Progress page's own `analytics.streaks.current`, since both trace back to the same function. |
| `hasDigestContent` | `(digest) => boolean` | `true` iff `completedCount > 0 \|\| streakDays > 0` — gates whether the banner renders at all; a week with nothing to report shows nothing. |
| `formatDigestMessage` | `({ completedCount, streakDays }) => string` | Plain-language sentence per `.claude/rules/content-style.md` ("topics," second person, terminal punctuation) — e.g. "You completed 12 topics this week. You're on a 3-day streak." Streak folds into the same sentence rather than a separate fragment; omitted entirely when `streakDays <= 0`. |

The "already shown this week" guard (`src/ui/utils/progressDigest.js`'s
`shouldShowProgressDigest`/`markProgressDigestShown`, backed by a per-uid
`progressDigestLastShownKey(uid)` in `localStorageKeys.js`) is device-local only, never
synced to Firebase, and uses a rolling 7-day interval rather than a one-shot boolean —
see `.claude/rules/roadmap-store.md` for the full guard/banner-wiring writeup.
