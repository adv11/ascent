...
- PWA/meta: favicon + app icons = red triangle on `#F3F2F2` (dark: on `#141312`); `<meta name="theme-color">` = `#EC3013`; og-image rebuilt in Archivo 800 with the red poster style. Icon URLs (`index.html`'s `<link rel="icon"|"alternate icon"|"apple-touch-icon">` and `public/manifest.json`'s `icons[].src`) carry a `?v=N` cache-busting query string — mobile browsers/OSes cache a home-screen icon at install time and never re-fetch it just because the file at the same URL changed server-side (issue #346). Bump `N` on every one of these URLs, in the same PR, whenever `scripts/generate-brand-assets.mjs` is re-run for a brand color/mark change — see that script's own header comment. **`manifest.json` itself needs the same "won't silently update" caution, one layer up (issue #403).** The versioned icon-file query strings above only protect the icon *files* — `manifest.json` names those URLs but has no versioned URL of its own, and used to fall under `firebase.json`'s blanket `/public/**` rule (`Cache-Control: max-age=31536000, immutable`) plus `sw.js`'s cache-first precache list, either of which could keep a device serving a *stale manifest* (and therefore a retired icon URL) for up to a year even after every icon file's own version bump. `firebase.json` now carves `manifest.json` out with its own short-lived `Cache-Control` (matching `index.html`'s), and `sw.js` routes it through `networkFirst` instead of precaching it cache-first — reported live as a device's native "Install app" prompt still showing the old gold triangle. If you ever add another file that *names* other versioned assets rather than being one itself (a second manifest, an asset map), give it this same "always revalidate" treatment rather than letting it inherit whatever caching rule its directory happens to have.
