...
**"Next up" card selection — `src/core/roadmap/nextUp.js` (issue #491).** `selectNextUpTopics(items, phases, { limit, excludeIds })` is a **pure** module (no DOM/store access), same "independently unit-testable" precedent as `reviewSchedule.js`/`completionCelebration.js`/`roadmapComparison.js` above. Selection order: (1) unfinished topics in the phase containing the most recently completed topic (found via `item.completedAt`, the same field `reviewSchedule.js` reads); (2) once that phase has no unfinished topics left (or nothing's ever been completed), the first unfinished phase in **template order** — `phases` (the store's `snapshot.phases` skeleton) supplies that order; falling back to first-encountered-in-`items` order only if `phases` is empty (a custom roadmap with no skeleton yet). Within a phase: Must do (P0) before Should do (P1) before Later (P2/P3, one shared tier), source order otherwise — `PRIORITY_RANK`'s `{ P0: 0, P1: 1, P2: 2, P3: 2 }` map. Returns `{ topics, complete }` — `complete` is only ever true when every topic in the roadmap is done; an `excludeIds`-driven empty result (every remaining candidate skipped by "Not today," below) returns `complete: false` with an empty `topics` array, never conflating "nothing left after exclusions" with "the roadmap is finished." `dashboard.js`'s `updateNextUpCard()` calls this from both `render()` and `patchDoneStates()` (the cosmetic-done-toggle fast path — a done toggle is exactly what changes this card's own selection, same reasoning `updateReviewDueBadge()` already documents) and renders up to `NEXT_UP_LIMIT` (3) rows reusing `.check-box`/`.check-mark`/`.check-body`/`.check-title`/`.check-meta` verbatim from `renderItemRow()`'s real checklist rows, so a "Next up" row is visually identical to its counterpart below minus the priority left-edge stripe and ⋮ menu. Ticking a row calls the existing `toggleDone()` directly — no new store surface area, no bespoke completion path. "Not today" (`nextUpExcludedIds`, a plain in-memory `Set` declared alongside `dashboard.js`'s other per-mount `let` state) adds the current three topics' ids to the exclusion set and re-calls `updateNextUpCard()` immediately — session-only, never persisted to `localStorage`/Firebase, and never marks anything done; a topic drops out of the exclusion set's effective reach the moment it's completed anyway, since `selectNextUpTopics` only ever draws from unfinished topics in the first place.
