...
**A cluster of small, closely-related blocks reads as clutter if each one gets its own separate border/card — group them inside one glass `.card` instead (issue #460).** Real, live feedback (screenshot): the dashboard header's roadmap identity badge, meta row, two stat tiles, and Priority/Tags filter rows were five-plus independently-bordered, independently-margined blocks stacked with their own gaps — functionally fine, but visually scattered rather than one cohesive header. The fix wasn't new visual language, just consolidation: `.roadmap-summary-card`/`.roadmap-filters-card` (`app.css`, `dashboard.js`) wrap the previously-separate `.roadmap-header`/`.stat-strip` and the two `.toolbar` rows in one `.card` each (the existing v3 glass primitive — no new tokens, no bespoke surface styling), with a hairline `border-left`/`border-top` divider *between* the grouped sub-elements standing in for what used to be each one's own full border. **The rule going forward**: when 2+ small, topically-related blocks sit adjacent with no unrelated content between them (a header's identity+stats, a settings section's related fields, a stat row next to its own filter controls), default to one shared `.card` wrapper with internal dividers over several independently-bordered mini-cards — reserve separate `.card`s for content that's genuinely independent (different data, different actions, meant to be visually distinct units). A nested element that used to have its own border/background (`.stat-tile` here) should drop it once it's inside the shared card — a border *inside* a border is exactly the "box inside a box" pattern this fix removes, not a look to preserve. Verify the consolidated card doesn't overflow at the narrowest real phone widths (~320px, iPhone SE-class) specifically — content that fit comfortably as two separate full-width blocks can still overflow once squeezed side-by-side inside one card at that width (found live via a real `document.documentElement.scrollWidth` check, not eyeballing); stack sub-elements vertically below whatever breakpoint that turns out to be, same as `.roadmap-summary-stats`' `≤480px` override does here.

**A plain `flex-wrap` row with no explicit grouping wraps wherever it runs out of space, not where the design intends — force a deliberate line break with `order` + `flex-basis: 100%` on the item that should start the new line (issue #463, retired by issue #488).** Real, reported clutter: `.app-topbar`'s narrow-viewport layout (hamburger, breadcrumb, review/todo badges, sync-pill, "Create account" button, icon group) used to be one `flex-wrap` container with no per-item `order` — each control wrapped onto whichever row it happened to run out of width on, producing up to three uncontrolled, visually unrelated-looking rows. The fix at the time grouped the controls that should share a line into `.app-topbar-status`, with explicit `order`/`flex-basis: 100%` pulling that group onto its own guaranteed row. **Issue #488 removed the underlying crowding instead of continuing to manage it**: the topbar is now just breadcrumb + `.icon-btn-group` (search + one avatar button) at every width, so `.app-topbar-status` and its `order`/`flex-basis`/`:empty` rules are deleted outright — there is no longer a second row to coordinate. This pattern (group + `order` + `flex-basis: 100%` + a `:empty` guard) remains the right fix if a *future* topbar/toolbar row ends up with 3+ unrelated controls fighting over one responsive row again — just don't assume the pattern itself is still wired up anywhere in `.app-topbar` today.

**Topbar contract (issue #488, bell re-split back out in issue #503/E3) — breadcrumb + search + bell + one avatar button, nothing else.** `createTopbar({ breadcrumb, user, store, dailyTodoStore, onDeleteAccount, onStartTour })` still doesn't take `syncPill`/`themeToggleBtn`/`dailyTodoNavBadge`/`reviewDueBadge` props — review-due/daily-todo badges and the sync pill are computed in `dashboard.js` but deliberately left unattached to any rendered DOM (a `// Issue #488` comment marks each), pending issue #489's single summary-card redesign giving them a new home. **The bell is its own icon button again** (`src/ui/components/notificationBell.js`, new module) — issue #488 had briefly folded it into the avatar dropdown (an unread dot on the avatar itself, plus a "What's New" item in the dropdown) as an interim call ahead of this issue landing; #503's own design reference showed a separate dot-badged bell beside the avatar, not folded in, so that fold is reverted. `notificationBell.js` owns its own unread-changelog dot and opens `changelogDrawer.js` exactly the way the folded-in version did (same `changelogSeen.js` persisted "last seen version" contract) — `topbar.js` just composes it into the icon group instead of building the logic inline. The avatar's dropdown is still `sidebar.js`'s own `buildAccountMenu()` (exported, taking an optional `onOpenChangelog` and `align` param) — the exact same Settings/tour/My reports/Share/backup/print/delete-account item list the sidebar footer already builds, reused rather than duplicated, and still keeps its own "What's New" entry as a second, harmless way to reach the drawer. This is deliberate, not incidental: below the sidebar's 900px breakpoint there is no sidebar footer at all (`bottomNav.js` is the nav instead), so the topbar's avatar is the *only* remaining account-menu entry point at that width — at >=900px it's a second, harmless way to reach the same menu. Theme lives in the sidebar footer (`createThemeToggle()`, a plain sibling of the sign-out button, sized identically via the shared `.btn-icon` 36px rule — no new CSS needed for the expanded state, only `.app-sidebar.collapsed`'s icon-rail sizing override needed a matching `.theme-toggle.btn-icon` selector) and remains in Settings' own theme `<select>` (`settings.js`, unchanged). The guest "Local only"/"Create account" pair is `guestBanner.js`'s `createGuestBanner(user)` — same dismissible-banner shape as `verificationBanner.js` (`.guest-banner`, `.guest-banner-msg`, `.guest-banner-dismiss`) but `sessionStorage`-keyed (`KEYS.GUEST_BANNER_DISMISSED`, no per-uid keying) so it reappears on a fresh session rather than being dismissed forever on a device — rendered in `.app-content`, above the page's own header, on every page that renders a topbar (dashboard/settings/progress).
...
**Layout primitives — `--content-max`, `--tap-min`, fluid heading type, and the 16px body-copy floor (`src/styles/app.css` `:root`, issue #482 A1).** The one shared place every other responsive-redesign issue in this batch (#483 onward) builds on. `--content-max: 1080px` + `.app-content` (`max-width: var(--content-max); margin: 0 auto; padding: clamp(14px, 3vw, 32px);`) is the only place page width is capped — nothing stretches edge-to-edge on a large monitor. `--text-2xl`/`--text-3xl`/`--text-4xl` are `clamp()` pairs now, not fixed sizes — same token names, so no call site changed, but page headings scale continuously from 360px to 1920px instead of jumping at a breakpoint; any *new* page-heading size should follow the same `clamp(min, preferred-with-vw, max)` shape rather than a fixed rem value. `--tap-min: 48px` plus a base rule on `.btn`/`.filter-chip`/`.nav-item`/`.check-box`/`.seg-opt`/`.btn-icon` floors every one of those to a 48×48px hit area regardless of pointer type — this sits *underneath*, not in place of, the existing `(pointer: coarse)` 46px overrides from issue #36/#233 above; both apply, 48px always wins since it's the larger value. **Nothing may render below `--text-base` (16px) except `.eyebrow`, `kbd`, and table headers (`th`)** — every rule that used to read `var(--text-xs)`/`var(--text-sm)` for `font-size` was raised to `var(--text-base)` as part of this same issue, including `.btn-sm` and `.priority-table` (both previously undercut the floor). `tests/unit/typeScaleFloor.test.js` regex-scans `app.css` for this and fails CI if a new sub-16px rule is added outside the three exempt selectors — check it before introducing a new small-text class rather than reaching for `--text-sm`/`--text-xs` on anything that reads as body copy.
...
**Bottom-sheet modal contract below 720px (issue #497, D1 of the responsive-redesign batch) — implemented once in the shared shell, never per-component.** Below 720px, `.modal-overlay` switches from `align-items: safe center` to `align-items: flex-end; padding: 0` and `.modal-card` docks to the bottom of the screen: full width, `border-radius: 22px 22px 0 0`, `max-height: 92dvh` with `overflow-y: auto` on the card itself (content scrolls, never the overlay), and a drag-handle bar drawn as a `.modal-card::before` pseudo-element — deliberately not a real DOM node, so every modal built on `.modal-card` (confirmDialog, commandPalette, importRoadmapModal, shareRoadmapModal, deleteAccountModal, feedbackModal) gets the handle automatically with no JS change. `.item-panel` (which predates `modal.js` and uses its own `.panel-overlay`/`.panel-header` shell, not `.modal-card`) gets the identical shape via its own rule in the same `@media (max-width: 720px)` block, with the handle on `.panel-header::before` instead. This widened the item panel's previous ≤480px threshold (issue #6 Phase 8) so every modal in this batch switches to a sheet at the same viewport width — don't reintroduce a narrower or different breakpoint for a new modal without a specific reason. Overriding `align-items` to `flex-end` here does **not** reintroduce the clipping trap the paragraph above warns about: the card's own `max-height: 92dvh` plus `overflow-y: auto` caps and scrolls its content independently of the overlay, so there's nothing left for `flex-end` to clip. Close buttons (`.modal-close`, `.feedback-modal-close`) grow to the 48px `--tap-min` floor in this same block, unconditional on pointer type — a deliberate exception to this file's usual `(pointer: coarse)`-gated touch-target rule, since the design reference ties the 44px close-target minimum to viewport width, not touch capability. Primary actions in `.confirm-dialog-actions`/`.feedback-form-actions` stack (`flex-direction: column`) and go full width; any modal already using `.btn-block` on its primary action (`deleteAccountModal.js`, `importRoadmapModal.js`) needed no change, since that class is already unconditionally full width. If you add a ninth modal on top of the eight this issue covers, building it on `.modal-overlay`/`.modal-card` (or `.panel-overlay`/`.item-panel`) is what makes it sheet-ready for free — a modal that invents its own overlay/card markup gets none of this.
...
**Text size and animations-off preferences (`src/services/uiPreferences.js`, issue #495, text-size mechanism revised by issue #496 follow-up) — settings.js's Preferences tab, applied app-wide via a root `data-*` attribute, no per-component wiring.** Text size (`Default`/`Large`/`Largest`) is a `data-text-size` attribute on `<html>`, read by two CSS rules in `app.css` that set a `--text-scale` custom property (`html[data-text-size='large'] { --text-scale: 1.125; }`, `'largest'` → `1.25`) — every `--text-*` token is wrapped in `calc(... * var(--text-scale))`, so only font sizes respond to this preference. **This used to set `font-size` directly on `<html>`** (scaling the root font size, which every `rem`-based token in the file — not just `--text-*` — responds to); that was changed after a real, reported bug: `--space-*`/`--radius-*`/card-padding tokens are also `rem`-based, so "Largest" was inflating gaps, icon tiles, and card padding right along with the type scale, most visibly as icon/badge crowding on the onboarding roadmap grid. Any *new* font-size rule must stay `rem`-based (reading a `--text-*` token, not a raw `px` value) or it silently won't scale with this preference — the exact same discipline the `--text-base` 16px floor rule above already requires, just for a different reason. A *new spacing/layout* token, by contrast, must **not** route through `--text-scale` — only genuine type-scale tokens should ever read it. Animations-off is a plain boolean `data-animations-off` attribute (present/absent, no value), **wired up in issue #499 (D3)'s motion pass** — a global `[data-animations-off], [data-animations-off] *, [data-animations-off] *::before, [data-animations-off] *::after { animation-duration: 0.01ms !important; transition-duration: 0.01ms !important; scroll-behavior: auto !important; }` rule, the same shape as this file's `prefers-reduced-motion: reduce` catch-all, not a per-component opt-out — so every CSS animation/transition in `app.css` is suppressed the moment the preference is on, including any added after this pass. A CSS rule can't reach JS-driven animation (e.g. `countUp.js`'s rAF-based count-up), so that one checks `document.documentElement.hasAttribute('data-animations-off')` directly, alongside its existing `prefers-reduced-motion` check — any future rAF/`setInterval`-driven visual effect needs the same explicit check, not just a CSS override. Both attributes are set synchronously by `themeBootstrap.js` (the same classic, non-module `<script>` that sets `data-theme` before CSS loads) so a reload never flashes the default text size or briefly plays an animation the user turned off — if you add a third settings-driven preference that needs to affect first paint, extend `themeBootstrap.js` the same way rather than applying it later from a page-level script.
...
**`.check-item-focused` is a purely visual keyboard-navigation ring, distinct from `:focus-visible` (issue #379).** The dashboard checklist's `j`/`k` row navigation moves a plain `.check-item-focused` outline class between rows (tracked by `dashboard.js` via a plain item id, not real DOM focus, since `.check-item` rows aren't themselves focusable elements) — this is a different concept from `.check-item:focus-visible`/`.check-box:focus-visible`, which still marks genuine keyboard focus reaching a real focusable control. Both can be present at once (tabbing to a `.check-box` doesn't clear the `j`/`k` ring, and vice versa) — don't try to unify them into one class.
...
**`attachFocusTrap()` only cycles Tab within an already-open dialog — it never moves focus into the dialog on open. That's always the caller's own job (issue #357).** `openModal()`'s own builder does this automatically for anything built on top of it, but four hand-rolled `attachFocusTrap()` call sites (`feedbackModal.js`'s type-select screen/"My reports" tab/success screen, the standalone `myReports.js` modal, `buildYourOwnGuide.js`, `dailyTodoGuide.js`) shipped without a `.focus()` call of their own — a real, live-shipped a11y regression, not a hypothetical: a keyboard/screen-reader user opening any of these had focus stay on whatever triggered the dialog (now visually covered by the overlay), and the Tab-trap didn't meaningfully engage until they happened to Tab into it by chance. Every other hand-rolled modal in this codebase already gets this right (`confirmDialog.js`, `importBackupModal.js`, `deleteAccountModal.js`, `addToDailyTodoModal.js`, `roadmapComparisonModal.js`, `commandPalette.js`, `featureTour.js`'s welcome card) — call `someEl.focus()` on a sensible first element (a primary action button, a "Back"/close control) immediately after `attachFocusTrap()` and appending the overlay, every time you write a new hand-rolled dialog. Easy to forget precisely because the Tab-cycling half of the behavior looks correct without it — check this specifically on every new modal, not just that Escape/Tab work.

**Any footer/row that stays visible in an icon-only sidebar rail must switch to a vertical stack, not shrink a horizontal row (issue #102).** `.app-sidebar-footer` (avatar identity + sign-out button) is a horizontal flex row sized for the full 240px sidebar — both icon-only rail states (the automatic 640–1023px tablet rail, 56px wide, and the manual desktop `.collapsed` rail, ≥1024px, 64px wide) are too narrow for that row once the email label is hidden, and the two controls visually overlapped in both instead of wrapping (found live, screenshot report). The fix is `flex-direction: column` on the footer plus shrinking the sign-out button to match the avatar's size, applied identically under `.app-sidebar.collapsed` and inside the `≤1023px` tablet media query — never try to solve this by shrinking/squeezing a horizontal row into a narrower one, since a `flex: 1` + `flex-shrink: 0` sibling pair just overlaps rather than compressing. **This paragraph's `640–1023px` automatic icon rail and the mobile hamburger-drawer mechanism it describes are retired as of issue #484** — see the note immediately below.

**The sidebar/bottom-nav split — sidebar at >=900px, `bottomNav.js` below it, no rail or drawer in between (issue #484).** The old three-tier mobile nav story (a 640–1023px automatic icon-only rail, a <640px off-canvas drawer opened by a hamburger button, and a manual desktop `.collapsed` rail at ≥1024px) is replaced by a hard two-way split: `.app-sidebar` renders in full at ≥900px (the manual desktop `.collapsed` icon-rail toggle is unchanged and still lives entirely inside that range) and is `display: none` below it — no icon-only rail, no drawer, no hamburger. Below 900px, `src/ui/components/bottomNav.js` renders a fixed bottom tab bar instead (four tabs: Dashboard/Progress/My Roadmaps/Settings, `56px` + `env(safe-area-inset-bottom)`, each tab a `--tap-min` 48px target, active tab `--color-accent-100` background + `--color-accent-700` text) — mounted as a shell-level sibling in `dashboard.js`/`progress.js`/`settings.js` (not `onboarding.js`, which has no sidebar to begin with, see its own "no app-shell sidebar" note below) alongside the existing `sidebar`/`topbar`, with its own `_cleanup` wired into the route's cleanup return like every other shell component. `sidebar.js` no longer has `_toggleMobile`/`_openMobile`/`_closeMobile`/`_backdrop` at all, and `topbar.js` no longer has a hamburger button or an `onToggleMobileSidebar` prop — if you find a stray reference to any of these, it's a regression from before this issue, not a mechanism still in use. `featureTour.js`'s dashboard walkthrough used to force-open the mobile drawer around any step flagged `requiresMobileSidebar` (issue #349) — that flag and its `onOpenSidebar`/`onCloseSidebar` hooks are gone; a step whose `target()` now resolves to `null` (because its element genuinely isn't rendered at the current viewport, e.g. the sidebar's own account-menu trigger below 900px) is skipped forward to the next step automatically instead of ending the tour early, and dashboard.js's sidebar-nav-item tour steps fall back to their `.bottom-nav-item` equivalents when the sidebar isn't present. `.app-content` gets `padding-bottom: calc(var(--bottom-nav-h) + env(safe-area-inset-bottom) + var(--space-4))` below 900px so page content never renders underneath the fixed bar.
...
**Not every `color-contrast` finding is the sampler false-positive above — `.feedback-widget-trigger`/`-label` was a real, unfixed regression, found only because CI's deploy-gating (issue #266) quietly did its job (issue #385).** PR #337's own CI run failed this exact axe check (`.feedback-widget-label` at 3.42:1, "serious" impact) — `deploy.yml` correctly skipped deploying that merge commit, and the very next merge happened to pass CI and re-deploy everything a short time later, so nothing broke in production. But the underlying bug itself was never actually fixed — it just rode along silently in every deploy since, because nothing surfaces a "CI failed once, then the next push happened to fix it as a side effect" case as its own issue. Root cause: `.feedback-widget-trigger` set `background: var(--color-accent)` (the *base* accent) under `color: var(--color-ink-on-accent)` — the identical contrast bug `.btn-primary` already hit and fixed by reading `--color-accent-600` (a darkened fill) instead of the base accent whenever accent-colored text-hosting content sits on top of it (see that rule's own code comment in `app.css`, directly above `.feedback-widget-trigger`). Live-verified: light theme's base accent only clears ~3.76:1 against `--color-ink-on-accent` (below the 4.5:1 WCAG AA floor for bold 14px text); `--color-accent-600` clears ~4.94:1. Dark theme improves too (5.88:1 → 6.69:1), not just light — verify both themes' actual numbers whenever you copy this fix pattern elsewhere, the same discipline every other token pairing in this file follows. **The lesson**: a CI failure that "resolves itself" on the next push is not evidence the regression was fixed — it means the *next commit's own unrelated changes* happened to also get deployed together with the still-broken one. Audit what a failing run actually flagged before assuming a later green run means everything's fine.
...
**Dashboard checklist rows are windowed/virtualized while a phase-card is open and scrolled off-screen — row-level, not phase-card-level, and DOM-node-preserving, not DOM-removing (issue #433, follow-up to #432/#430).** A real, reported bug persisted after #432's `[data-scrolling]` fix (which only ever addressed `backdrop-filter` compositing cost): fast/sustained scrolling through a fully-expanded 19-phase-card Java Backend roadmap (484 topic rows, ~37800px tall page) still showed large black/unpainted regions. The issue's own live isolation test — stripping `backdrop-filter`/`box-shadow`/`border-radius`/`overflow:hidden` from every `.phase-card` and repeating the fast-scroll test — reproduced the identical black-flash, ruling out glass compositing as the (sole) driver and pointing at raw paint/raster volume from hundreds of always-mounted `.check-item` rows instead.

`content-visibility: auto` was tried a **second** time for this symptom (row-level, `.check-item`, with a real measured `contain-intrinsic-size: 67px` this time — the first, card-level attempt documented in `app.css`'s own `.phase-card` comment, issue #430, had neither a narrower unit nor any `contain-intrinsic-size` at all) and, despite being a more careful setup, still measurably made the black-flash *worse*, not better, confirmed via two independent scripted repro methodologies (a fast synthetic jump-scroll and a realistic ~60fps-paced mouse-wheel scroll) — both showed a higher worst-frame blank-content fraction with row-level `content-visibility` than without it. The likely reason: `content-visibility: auto` on ~500 individual small elements makes the browser run its own intersection-relevance bookkeeping for every one of them on every scroll frame, and that per-element overhead outweighs the paint it saves for content this cheap to paint in the first place — a different failure mode than the card-level attempt's total-repaint-never-recovers regression, but the same practical outcome: don't ship it. **If you're tempted to try `content-visibility: auto` on this checklist a third time, at any granularity, measure it against a real before/after scripted repro first** — it has now failed twice, for two different reasons, and "the previous attempt just didn't set `contain-intrinsic-size` / didn't apply it narrowly enough" is not sufficient justification to expect a different result on a third try.

The fix that actually held up under repeated measurement: real windowed/virtualized rendering of `.check-item` rows, built as simply as this codebase's no-framework/no-build-step constraint allows. `dashboard.js`'s `renderPhaseCard()` wraps each section's item rows in a `buildSectionRows()`-built `.section-rows` container (a top `.row-spacer`, the rows, a bottom `.row-spacer`) instead of rendering them as bare siblings under `.phase-body`. A `scroll`/`resize`-driven pass (`virtualizeOpenSections()`, rAF-throttled via `scheduleVirtualizeRows()`) recomputes, for every currently-**open** phase-card's each section, which row indices fall within roughly `VIRTUALIZE_BUFFER_VIEWPORTS` (2.5) viewport-heights of the current scroll position and reconciles the DOM to match (`syncSectionRowsWindow()`) — pruning far rows out (capturing each pruned row's *real* `getBoundingClientRect().height` into a per-row `_rowHeights` array before removing it, so the replacing spacer's height is measured, not just estimated) and mounting newly-in-range rows back in. This is an "estimate then correct from real measurements" scheme, not a fixed-height virtual list: `ROW_HEIGHT_ESTIMATE` (67px, measured live from every row in the fully-expanded Java Backend template) is only ever used for a row that's never actually been mounted and measured yet.

Every row is still a real, always-present DOM node the moment it matters, which is what keeps every other convention in this app working unmodified with zero changes elsewhere:
- **`j`/`k` keyboard nav** (`getVisibleRows()`, issue #379, `content.querySelectorAll('.phase-card.open .check-item')`) queries whatever's currently mounted, exactly as documented for that feature already ("rows already on screen") — a row far outside the buffer simply isn't in that query's result set, same as it was already true that a collapsed phase's rows never were. `setRowFocus()`'s own `scrollIntoView()` call, and Playwright's auto-scroll-into-view before any interaction, both dispatch a real `scroll` event that re-mounts the target row before anything tries to click it.
- **E2E tests that locate a row by `.check-item[data-id="…"]`** are unaffected the same way — the row exists in the DOM the moment the test's own scroll (real or Playwright-driven) brings it within the buffer.
- **`animatePhaseBody()`'s FLIP open/close height animation** (issue #6 Phase 7) is never affected: pruning only ever runs off a `scroll`/`resize` event (never during the animation itself), and is explicitly skipped for any section whose ancestor carries `.phase-body-animating` — so `scrollHeight` is always measured against a fully-mounted body, never a partially-pruned one caught mid-animation by an unlucky scroll event.
- **`patchDoneStates()`'s** existing `if (!row) return;` guard (already there for other reasons) silently no-ops for a currently-pruned row; it picks up the correct `done` state the moment that row is mounted again, since mounting always reads current store state.

Measured result (same scripted repro, same before/after steps): a fully-expanded 19-phase roadmap's mounted DOM node count drops from ~11,200 to ~1,500; worst-frame blank-content fraction (pixel-sampled against the page's own background color, chrome excluded) drops to 0% under a realistic ~60fps-paced mouse-wheel scroll, and stays flat under an aggressive fast-fling scroll compared to before the fix. **One residual, deliberately-not-chased finding**: an extreme, faster-than-any-real-input-device synthetic wheel burst (zero settle time between ticks) still shows an identical blank-frame artifact before and after this fix — the same magnitude, at the same scroll position, regardless of whether row virtualization is applied at all. This matches the issue's own isolation-test finding (stripping every visual effect from `.phase-card` didn't fix that extreme case either) and points at a Chromium GPU-compositor raster-scheduling limit under synthetic scroll input no real human gesture produces, not something any DOM/CSS-level page fix can reach — flagged here so a future investigation doesn't re-attempt the same isolation test a third time expecting a different result at that specific extreme input rate.

**`ROW_HEIGHT_ESTIMATE` (67px) is now only a cold-start fallback, not the estimate used everywhere — a real recurrence of the blank-flash bug on mobile, issue #450.** #444's mobile checklist-row layout (`.check-item { flex-wrap: wrap }`, taller wrapped rows under `≤480px`) made real mobile row heights diverge substantially from the desktop-measured 67px constant, but the spacer sizing (`sumRowHeights()`) and scroll-position-to-row-index math (`virtualizeOpenSections()`) both used that fixed constant unconditionally for any row not yet individually measured — every row starts unmeasured, so on a phone the spacers stayed systematically undersized until enough rows were measured (only happens the moment a row is pruned out of the mounted window), and the self-correction was a sudden, visible layout jump read as a momentary blank/disappearing region during a fast scroll. Fixed by tracking a running average of every row height actually measured so far (`estimateRowHeight()`/`recordMeasuredHeight()`, `dashboard.js`) and using that average — not the fixed constant — as the estimate for any unmeasured row; `measureMountedRows()` seeds the average from whatever's already mounted the first time a section is virtualized, so the very first scroll on a phone already uses a mobile-accurate estimate instead of the desktop-tuned one. `_rowHeights` is now seeded with `0` ("unknown"), not the constant — `sumRowHeights()`'s fallback parameter only ever applies to a falsy entry, so seeding it with a truthy placeholder (the original shape) would have made that fallback unreachable for any row that hadn't been individually measured yet, silently reintroducing the same bug this fix closes. If you add a fourth breakpoint-specific checklist-row layout change, re-check whether it changes real row height meaningfully — this adaptive estimate self-corrects, but only once enough rows have actually been measured; a layout that's dramatically taller only on the very first screenful (before any row has been pruned/measured) would still cold-start from the 67px constant for that first render.

**A same-symptom-looking but structurally distinct bug: `.phase-card.open`/`[data-scrolling]`'s flat fallback fill reading meaningfully lighter than the page background, not a virtualization/paint bug at all (issue #455).** A live report (screenshots, dark theme) described "background/content flashing grey while scrolling" and was initially assumed to be another recurrence of the #430/#432/#433/#450 blank-paint chain immediately above — it wasn't. Root cause: `.phase-card.open` and the `[data-scrolling]` scroll-perf fallback (`app.css`, `scrollPerfMode.js`, issue #432) drop `backdrop-filter` for a flat fill, and that fill read `--color-surface-raised` — a real fix issue #435 shipped at the time for a genuine `--v3-surface` hue mismatch (`--v3-surface` was then a cool blue-gray, unrelated to this app's warm-neutral `--color-*` palette). Issue #440 later retuned `--v3-bg`/`--v3-surface`/`--v3-surface-elevated` to the exact HSL equivalent of the real `--color-bg`/`--color-surface` hex values, closing that mismatch — but *deliberately* left card-shaped surfaces on `--color-surface-raised` anyway, on the stated theory that "these are meant to read as distinct cards regardless of scroll state" (unlike the topbar/sidebar, which #440 already moved to `--color-bg` for the identical "reads as a bug" reason). #455's live feedback says that theory doesn't hold once a long, expanded roadmap has a dozen-plus open phase-cards simultaneously visible: `--color-surface-raised` is genuinely lighter than both the page background and these surfaces' own resting translucent glass fill, and the whole visible page flashing that color on every scroll reads as a bug. Fixed by pointing both rules at opaque `hsl(var(--v3-surface))` instead — the same hue the translucent resting state already reads against the near-black page, so dropping `backdrop-filter` is now visually seamless. **The lesson for a future investigation**: a "grey/lighter flash while scrolling" report and a "blank/unpainted region while scrolling" report can look identical in a screenshot but have completely different root causes (a color-token mismatch on an always-correctly-sized surface vs. a genuinely missing/mis-sized DOM region) — check the resolved `background-color` of the visible glass surfaces first (cheap, deterministic, `getComputedStyle`) before assuming it's another row-virtualization regression and re-auditing `dashboard.js`'s spacer math. `tests/e2e/scrollBackgroundFlash.test.js` is the scripted regression test for this specific color-mismatch class, following the exact "before/after repro, sampled against the page's own background" discipline #433/#450's own investigations established, just checking resolved color instead of pixel-sampled paint.
