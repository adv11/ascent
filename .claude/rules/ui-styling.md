...
**Topbar contract (issue #488) — breadcrumb + search + one avatar button, nothing else.** `createTopbar({ breadcrumb, user, store, dailyTodoStore, onDeleteAccount, onStartTour })` no longer takes `syncPill`/`themeToggleBtn`/`dailyTodoNavBadge`/`reviewDueBadge`/`notificationBell` props — review-due/daily-todo badges and the sync pill are computed in `dashboard.js` but deliberately left unattached to any rendered DOM (a `// Issue #488` comment marks each), pending issue #489's single summary-card redesign giving them a new home. The bell folds into the avatar: `topbar.js` builds its own unread-changelog dot (`.avatar-unread-dot`, reusing `.notification-badge`/`.notification-badge-dot`'s existing styling) positioned over the avatar via `.app-topbar-avatar-btn .avatar-unread-dot`, and a "What's New" item inside the avatar's dropdown opens `changelogDrawer.js` the same way the old `notificationBell.js` did — that module (and its test) is deleted outright rather than left unused, since nothing imported it anymore once the bell folded into the avatar (`scripts/lint-theme.mjs` would otherwise flag its orphaned `.app-topbar-bell` class for a missing `color` rule the moment its CSS was removed). The avatar's dropdown is `sidebar.js`'s own `buildAccountMenu()` (now exported, taking a new optional `onOpenChangelog` and `align` param) — the exact same Settings/tour/My reports/Share/backup/print/delete-account item list the sidebar footer already builds, reused rather than duplicated. This is deliberate, not incidental: below the sidebar's 900px breakpoint there is no sidebar footer at all (`bottomNav.js` is the nav instead), so the topbar's avatar is the *only* remaining account-menu entry point at that width — at >=900px it's a second, harmless way to reach the same menu. Theme moved to the sidebar footer (`createThemeToggle()`, a plain sibling of the sign-out button, sized identically via the shared `.btn-icon` 36px rule — no new CSS needed for the expanded state, only `.app-sidebar.collapsed`'s icon-rail sizing override needed a matching `.theme-toggle.btn-icon` selector) and remains in Settings' own theme `<select>` (`settings.js`, unchanged). The guest "Local only"/"Create account" pair is `guestBanner.js`'s `createGuestBanner(user)` — same dismissible-banner shape as `verificationBanner.js` (`.guest-banner`, `.guest-banner-msg`, `.guest-banner-dismiss`) but `sessionStorage`-keyed (`KEYS.GUEST_BANNER_DISMISSED`, no per-uid keying) so it reappears on a fresh session rather than being dismissed forever on a device — rendered in `.app-content`, above the page's own header, on every page that renders a topbar (dashboard/settings/progress).
