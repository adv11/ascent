...
**A plain `flex-wrap` row with no explicit grouping wraps wherever it runs out of space, not where the design intends — force a deliberate line break with `order` + `flex-basis: 100%` on the item that should start the new line (issue #463).** Real, reported clutter: `.app-topbar`'s narrow-viewport layout (hamburger, breadcrumb, review/todo badges, sync-pill, "Create account" button, icon group) was one `flex-wrap` container with no per-item `order` — each control wrapped onto whichever row it happened to run out of width on, producing up to three uncontrolled, visually unrelated-looking rows (the hamburger alone, then a status-badge row, then the icon row) instead of a clean, predictable layout. The fix: group the controls that should share a line into one wrapper element (`.app-topbar-status`, `topbar.js`), give every top-level item an explicit `order` matching the *intended* visual grouping (not necessarily DOM order — `.icon-btn-group` is DOM-last but `order: 3` pulls it onto row 1 with the hamburger/breadcrumb), and give the wrapper that should start a guaranteed-new row both the highest `order` *and* `flex-basis: 100%` — a `flex-basis: 100%` item always claims a full line to itself, and since nothing else is ordered after it, it lands alone on that new line rather than sharing it with whatever follows. If the "row 2" wrapper can legitimately end up with no visible children on some page/state (`.app-topbar-status` is empty on `/settings`/`/progress` for a signed-in user with no active Daily Todo/review-due item, since those pages pass `syncPill: null`), pair this with a `:empty { display: none }` rule so an empty wrapper doesn't still reserve its own blank row via `flex-basis: 100%`'s margin/gap. Reach for this pattern any time 3+ unrelated controls share one responsive row and a plain `flex-wrap` produces an unpredictable line count — never assume "it'll wrap sensibly" without checking a real narrow-viewport screenshot first.
