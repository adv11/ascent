...
src/data/changelog.json, changelog.js  What's New changelog data + APP_VERSION (issue #20) — every entry is `{ version: number, date: "YYYY-MM-DD", items: [{ type: 'feat'|'fix'|'improvement', title, description }] }`; APP_VERSION is `Math.max()` over every entry's version, parallel to ROADMAP_VERSION. Bump by appending a new, higher-version entry — never edit an already-shipped one. See docs/api.md for the full schema and src/ui/components/notificationBell.js/changelogDrawer.js for the bell + drawer UI.
