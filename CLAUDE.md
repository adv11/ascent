...
- **`.claude/rules/design-system.md`** — same trigger as `ui-styling.md` above; the **binding v3 "portfolio-synced" visual identity** (glass surfaces, 3-step rounded radius, green accent, gradients/glow, Archivo type per issue #435's explicit product-owner decision to keep it over the spec's Sora/Outfit) — this file's exact tokens/type/structure/component rules take precedence over anything conflicting in `ui-styling.md` or the retired v2 "Modernist"/v1 styling it replaced. Every UI PR is gated on its review checklist. Rollout tracked in issue #416, fully shipped as of #431 — see issue #418 for the doc/screenshot catch-up this transition required.
...
src/ui/components/feedbackModal.js, feedbackForm.js, myReports.js  in-app feedback & bug reporting (issue #9, screenshot capture removed in #348; the old floating feedbackWidget.js trigger retired in #498 — "Send feedback"/"My reports" now live in the account menu and a Settings row instead) — see .claude/rules/roadmap-store.md; src/services/feedbackStore.js, feedbackRateLimit.js, src/core/feedback/  Firebase write + rate limit + pure schema/metadata
...
**Brand rules.** Never hard-code the product name as a string in any `.js` file — import `createBrandMark()` / `createBrandWordmark()` / `createBrandIcon()` from `src/ui/components/brand.js` for brand *elements*, or the `BRAND_NAME` constant when you need the bare string inside a sentence. The literal lives in exactly one place, `src/core/brandName.js` — a module with **zero imports**, so `src/core/**` (pure by contract) and the pure data modules can read it without pulling in `brand.js`'s DOM chain; `brand.js` re-exports it, so `import { BRAND_NAME } from './brand.js'` still works in UI code. `scripts/lint-brand.mjs` (CI, alongside `lint-theme`/`lint-icons`) fails the build on any other occurrence — comments are exempt. The only permitted occurrences of the literal string `'Ascent'` are `src/core/brandName.js` itself and `index.html`'s meta tags/title. All `localStorage`/`sessionStorage` keys must come from `src/services/localStorageKeys.js`'s `KEYS` object (or `verifyDismissedKey()`) — never write a raw `ascent-*` string in any other file. This is what makes a future rename (or a white-labeled variant) a one-file change instead of a repo-wide grep; see Issue #7 and `docs/adr/ADR-004-product-rename.md`.
